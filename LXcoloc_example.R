...
samplesize_eqtl_file= "meta_GCST_41-50_gwas_id_info.xlsx" #如果表里有，直接用；如果没有，则需要在下载数据时，查看相应信息


#-------gwas data (outcome data) ----------------------------------

#结局数据：可以是id或本地数据，但如果是vcf.gz格式，还需要下载其index文件（tbi格式）
outcomeID ="GCST90275041.tsv"  #结局数据
type2 = "cc"
...
samplesize_gwas= "NAFLD_gwas_id_info.xlsx" #如果表里有，直接用；如果没有，则需要在下载数据时，查看相应信息
...
# chrpos_test(n=10) # 在进行nontarget=T分析时，批量分析前n个的共定位情况
