...
#------locuscompare可视化-------------------
fn1_gwas <- data.frame(rsid=dataset2$snp,pval=dataset2$pvalues) %>% dplyr::arrange(rsid)
fn2_eqtl <- data.frame(rsid=dataset1$snp,pval=dataset1$pvalues) %>% dplyr::arrange(rsid)
