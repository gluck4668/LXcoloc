...
#---eqtl arguments-----
exp_file1 <- head(fread(dir(eqtlID,full.names = T)[1]))
...
#------sample information---------------------------------
sample_infor <- sample_info(eqtlID,samplesize_eqtl_file,outcomeID,samplesize_gwas)
sam_eqtl <- sample_infor$eqtl_sample %>% data.frame()
sam_gwas <- sample_infor$outcome_sample
...
if(class(samplesize_gwas)=="numeric")
  gwas_df$samplesize.outcome <- samplesize_gwas else {  #添加数字

    gwas_idx <- grep(substr(outcomeID,1,12),sam_gwas[,1])

    if(length(gwas_idx)>1){
      sub <- sam_gwas[gwas_idx,]
        if(length(table(sub$samplesize))>1)
          gwas_df$samplesize.outcome <-sum(sub$samplesize) else
            gwas_df$samplesize.outcome <- sub$samplesize[1]

       } else
        gwas_df$samplesize.outcome <- sam_gwas$samplesize[1]
    }
