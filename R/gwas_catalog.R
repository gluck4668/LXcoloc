...
gwas_cata <- function(dir_file,eqtlID,type1,outcomeID,type2,pval_gwas,eaf_gwas,
                         beta_gwas,se_gwas,SNP_gwas,chr_gwasm,pos_gwas,
                         samplesize_gwas,geneChr,geneEnd,number,SNP_PP_H4,nontarget){

#-----arguments-----------------------------
if(!dir.exists(eqtlID) | length(dir(eqtlID))==0)
  stop(paste0("The '",eqtlID,"' was not existed, or it was empty."))

#---eqtl arguments-----
exp_file1 <- fread(dir(eqtlID,full.names = T)[1])

exp_ind <- c(pval_eqtl,eaf_eqtl,beta_eqtl,se_eqtl,
             SNP_eqtl,chr_eqtl,pos_eqtl)

not_exp_ind <- paste0(exp_ind[!exp_ind %in% names(exp_file1)],collapse = ", ")
if(!not_exp_ind=="")
  stop(paste0("'",not_exp_ind,"'"," was not in the colnames of eqtlID data. Please check the exposure data."))

rm(exp_file1)

#---eqtl samplesize-----
sam_eqtl <- read.xlsx(samplesize_eqtl_file) %>%
            arrange(-samplesize) %>% distinct(.,id,.keep_all = T)

eqt_list <- str_extract(dir(eqtlID),".*(?=_)")
no_id <- !eqt_list %in% sam_eqtl[,1]
no_eqtl <- paste0(eqt_list[no_id],collapse = ",")
if(!no_eqtl=="")
stop(paste0(no_eqtl, " could not be found in the sample file '",samplesize_eqtl_file,"' "))

#---outcome arguments-----
out_f <- fread(outcomeID)

out_ind <- c(pval_gwas,eaf_gwas,beta_gwas,se_gwas,
             SNP_gwas,chr_gwas,pos_gwas)

not_out_ind <- paste0(out_ind[!out_ind %in% names(out_f)],collapse = ", ")
if(!not_out_ind=="")
  stop(paste0("'",not_out_ind,"'"," was not in the colnames of outcomeID data. Please check the outcome data."))

#---outcome samplesize-----
cls <- class(samplesize_gwas)

if(cls=="character" & grepl(".xlsx",samplesize_gwas)){
sam_gwas <- read.xlsx(samplesize_gwas)%>%
            arrange(-samplesize) %>% distinct(.,id,.keep_all = T)

is.out <- any(grepl(substr(outcomeID,1,12),sam_gwas[,1]))
if(!is.out)
  stop(paste0(outcomeID, " could not be found in the sample file '",samplesize_gwas,"' "))
}
...
file_type <- str_extract(exp_list[x,1],"(?<=\\.)[^\\.]+$")

if(grepl(file_type,"xlsx",ignore.case = T) | grepl(file_type,"csv",ignore.case = T))
   eqtl_df <- eval(str2expression(paste0("read.",file_type,"(exp_list[x,1])"))) else
...
idx <- grep(str_extract(exp_list[x,1],"(?<=\\/).*(?=_)"),sam_eqtl[,1])
...
if(cls=="character" & grepl(".xlsx",samplesize_gwas)){
  idx <- grep(substr(outcomeID,1,12),sam_gwas[,1])
  gwas_df$samplesize.outcome <- sam_gwas$samplesize[idx] #添加例数
}else {
  gwas_df$samplesize.outcome <- samplesize_gwas #添加数字
  }
...
#筛选共定位的位点
  ...
  if(nontarget)
    file=paste0(dir_file,"/coloc_SNP (",str_extract(dir(eqtlID)[x],".*(?=_)"),"-",outcomeID,") ",Sys.Date(),"(nontarget).csv") else
      file=paste0(dir_file,"/coloc_SNP (",str_extract(dir(eqtlID)[x],".*(?=_)"),"-",outcomeID,") ",Sys.Date(),".csv")
...
#------locuscompare可视化-------------------
  ...
  coloc_plot <- locuscompare(in_fn1 = fn1_gwas, in_fn2 = fn2_eqtl,
                             title1 = str_extract(outcomeID,".*(?=\\.)"), title2 = str_extract(dir(eqtlID)[x],".*(?=_)"))

  coloc_plot

  if(nontarget)
    plot01_file=paste0(dir_file,"/coloc_plot_locuscompare (",str_extract(dir(eqtlID)[x],".*(?=_)"),"-",outcomeID,") ",Sys.Date(),"(nontarget).png") else
      plot01_file=paste0(dir_file,"/coloc_plot_locuscompare (",str_extract(dir(eqtlID)[x],".*(?=_)"),"-",outcomeID,") ",Sys.Date(),".png")
...
#---------geni.plots可视化--------------------------------------------------
  ...
  if(nontarget)
    plot02_file=paste0(dir_file,"/coloc_plot_geni.plots (",str_extract(dir(eqtlID)[x],".*(?=_)"),"-",outcomeID,") ",Sys.Date(),"(nontarget).png") else
      plot02_file=paste0(dir_file,"/coloc_plot_geni.plots (",str_extract(dir(eqtlID)[x],".*(?=_)"),"-",outcomeID,") ",Sys.Date(),".png")
