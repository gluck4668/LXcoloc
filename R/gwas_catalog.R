...
message("---It was completed to get the sample information from GWAS Catalong online---")
message("------------------------------------------------------------------------------")
message("---Now ,it will begin to analyze the colocalization---------------------------")
#------foreach--------------------------------------------
...
#筛选共定位的位点
 ...
 out_id01 <- str_extract(outcomeID,".*?(?=\\.)")
  if(grepl("_",out_id01))
    id_out <- str_extract(out_id01,".*?(?=_)") else
      id_out <- out_id01

  if(nontarget)
    file=paste0(dir_file,"/coloc_SNP (",id_exp,"-",id_out,") ",Sys.Date()," (nontarget).csv") else
      file=paste0(dir_file,"/coloc_SNP (",id_exp,"-",id_out,") ",Sys.Date(),".csv")
...
#------locuscompare可视化-------------------
  ...
  if(nontarget)
    plot01_file=paste0(dir_file,"/coloc_plot_locuscompare (",id_exp,"-",id_out,") ",Sys.Date()," (nontarget).png") else
      plot01_file=paste0(dir_file,"/coloc_plot_locuscompare (",id_exp,"-",id_out,") ",Sys.Date(),".png")
...
#---------geni.plots可视化--------------------------------------------------
  ...
  if(nontarget)
    plot02_file=paste0(dir_file,"/coloc_plot_geni.plots (",id_exp,"-",id_out,") ",Sys.Date()," (nontarget).png") else
      plot02_file=paste0(dir_file,"/coloc_plot_geni.plots (",id_exp,"-",id_out,") ",Sys.Date(),".png")
